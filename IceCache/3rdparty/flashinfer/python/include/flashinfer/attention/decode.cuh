...
#define DEBUG_PRINT (threadIdx.x == 0 && threadIdx.z == 0)
...
__device__ __forceinline__ void compute_qk(const T* smem, uint32_t compute_stage_idx,
                                           ...
                                           state_t<vec_size>& st,
                                           const uint32_t num_heads = 0,                 // New parameter
                                           const uint32_t head_idx = 0,                  // New parameter
                                           const uint32_t* page_valid_entries = nullptr, // New parameter
                                           const uint32_t page_size = 0,                 // New parameter
                                           const uint32_t cur_page_indptr_begin = 0) {   // New parameter
...
#pragma unroll
    ...
    // Modified masking logic to handle both last page and partially filled middle pages
    uint32_t global_idx = iter_base + tz * tile_size + j;
    // First check the basic bounds (original condition)
    bool is_valid = (global_idx < iter_bound);

    // Additional check for partially filled middle pages
    if (is_valid && page_valid_entries != nullptr && page_size > 0) {
      // Calculate which page this element belongs to
      uint32_t page_idx = global_idx / page_size;
      uint32_t page_offset = global_idx % page_size;
      
      // Check if this position is valid within its page
      is_valid = (page_offset < page_valid_entries[(cur_page_indptr_begin + page_idx) * num_heads + head_idx]);

    }

    s[j] = is_valid ? s[j] : -5e4;

    // if (DEBUG_PRINT && j == 0 && page_valid_entries != nullptr) {
    //     uint32_t page_idx = global_idx / page_size;
    //     uint32_t page_offset = global_idx % page_size;
    //     uint32_t valid = page_valid_entries[(cur_page_indptr_begin + page_idx) * num_heads + head_idx];

    //     printf("[block %d, vec_size %d] global_idx=%u, iter_base=%u, page_idx=%u, offset=%u, page_valid_entries[%u]=%u, cur_page_indptr_begin=%u, head_idx=%u, k_vec[3] = %f, is_valid=%f\n",
    //           blockIdx.x, vec_size,
    //           global_idx, iter_base, page_idx, page_offset, (cur_page_indptr_begin + page_idx) * num_heads + head_idx, valid, cur_page_indptr_begin, head_idx, k_vec[3], s[j]);
    // }
...
__global__ void BatchDecodeWithPagedKVCacheKernel(
    ...
    float rope_rcp_theta, uint32_t* __restrict__ page_valid_entries) {
  auto block = cg::this_thread_block();
  sm_scale *= math::log2e;
...
#pragma unroll
    ...
    compute_qk<pos_encoding_mode, vec_size, bdx, bdy * tile_size_per_bdx>(
        ...
        paged_kv.num_heads, kv_head_idx, page_valid_entries, paged_kv.page_size, cur_page_indptr_begin);
    block.sync();
...
cudaError_t BatchDecodeWithPagedKVCacheDispatched(
    ...
    float sm_scale, float rope_scale, float rope_theta, uint32_t* page_valid_entries, cudaStream_t stream) {
  const float rope_rcp_scale = 1.f / rope_scale;
  const float rope_rcp_theta = 1.f / rope_theta;
