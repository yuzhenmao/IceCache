...
__global__ void _scatter_kernel(
    const __nv_bfloat16 *__restrict__ transit_buffer,       // [1, 2 * 32 * 4 = 256,
                                                     // 16 * 128 = 2048]
    __nv_bfloat16 *__restrict__ kvc_buffer_ptr,             // [81920, 2, 32, 16, 128]
...
void scatter_pages(torch::Tensor transit_buffer,  // [1, 2 * 32 * 4 = 256,
  ...
  CHECK_EQ(transit_buffer.scalar_type(), torch::kBFloat16);
  CHECK_EQ(kvc_buffer.scalar_type(), torch::kBFloat16);
  ...
  auto* transit_buffer_ptr =
      reinterpret_cast<__nv_bfloat16*>(transit_buffer.data_ptr<c10::BFloat16>());
  auto* kvc_buffer_ptr =
      reinterpret_cast<__nv_bfloat16*>(kvc_buffer.data_ptr<c10::BFloat16>());
