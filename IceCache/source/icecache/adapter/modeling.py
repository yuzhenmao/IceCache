...
def _icecache_prefill(
    ...
    **kwargs,
    ...
    if state.layer2budget[cur_id] is not None:
        ...
        projected = torch.matmul(key, state.proj_vec[:-1]).to(torch.float32).reshape(state.n_kv_heads, -1, 1).detach().cpu().contiguous()
...
def _icecache_decode(
    ...
    **kwargs,
    ...
    if do_recv_pf and state.use_dci:
        ...
        if budget is not None and kvc.n_pages > budget and state.use_dci:
